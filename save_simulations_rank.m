...
for mc = 1:Nmc
    ...
    for rank_i = 1:length(rank) 
        for synch = 1:2
            ...
            PrFromCorr_W = inv(Wpwr+0.05*trace(Wpwr)/size(Wpwr,1))*(eye(size(UcorrW_rnk,1))-UcorrW_rnk(:,1:rank(rank_i))*UcorrW_rnk(:,1:rank(rank_i))')*Wpwr;
